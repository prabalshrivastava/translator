//window.onload=langDetectionTriggered();

//for detecting language
//chrome.runtime.onMessage.addListener(langDetectionTriggered);

function langDetectionTriggered(){
    ...
    for (let index = 0; index < para.length; index++) {
        ...
        if(element.innerText!=undefined && element.innerText!=""){
            ...
            detectCurrLang(element.innerText);
            break;
        }
    }
}
function detectCurrLang(sampleText){
    var detectUrl = "https://translate.yandex.net/api/v1.5/tr.json/detect";
    var xhr = new XMLHttpRequest(),
    data = "key="+keyAPI+"&text="+sampleText;
    ...
    xhr.onreadystatechange = function() {
        ...
        if (this.readyState==4 && this.status==200) {
            ...
            if(json.code == 200) {
                ...
                languageDetected = getLanguageName(json.lang);
                console.log(json.lang,languageDetected);
                sendLanguageResponseToPopup(languageDetected);
            }
            else {
...
function sendLanguageResponseToPopup(languageDetected) {
    //chrome.tabs.sendMessage(tabs[0].id,languageDetected);
...
//for translating language
chrome.runtime.onMessage.addListener(gotMessage);

function gotMessage(message,sender,sendResponse){
    if(message === 'detectMyLang'){
        langDetectionTriggered();
    }else{
